...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    private Sprite unselectedPage;
    private Sprite selectedPage;
    ...
    [SerializeField] private Image scrollrectBG;
    private List<Image> _pageSelectionImages = new List<Image>();
    private List<Sprite> pageSprites = new List<Sprite>();
    private Image      imagePrefab;
    private Image      btnPrefab;
    private GameObject _perv_btnPrefab;
    private GameObject _next_btnPrefab;

    //------------------------------------------------------------------------
    public void Setup(
        List<Sprite> pageSprite,
        Image imagePrefab,
        Image btnPrefab,
        GameObject _perv_btnPrefab,
        GameObject _next_btnPrefab,
        Sprite unselectedSprite,
        Sprite selectedSprite,
        Sprite scrollrectBG
        )
    {
        this.pageSprites = pageSprite;
        this.imagePrefab = imagePrefab;
        this.btnPrefab   = btnPrefab;
        this._perv_btnPrefab = _perv_btnPrefab;
        this._next_btnPrefab = _next_btnPrefab;
        this.unselectedPage = unselectedSprite;
        this.selectedPage = selectedSprite;
        this.scrollrectBG.sprite = scrollrectBG;
    }
    public void ScrollSnapRectSetup() {
        ...
        {
            Image imagePrefab = Instantiate(this.imagePrefab,_container.transform);
            imagePrefab.sprite = pageSprites[i];
        }
    ...
    private void InitPageSelection() 
    {
        ...
        if (_showPageSelection) {
            // spawn prevBtn
            prevButton = Instantiate(this._perv_btnPrefab, pageSelectionIcons);
            // spawn pageBtn
            for (int i = 0; i < _pageCount; i++)
            {
                Image btnPrefab = Instantiate(this.btnPrefab, pageSelectionIcons);
                btnPrefab.sprite = unselectedPage;
                _pageSelectionImages.Add(btnPrefab);
            }
            // spawn nextBtn
            nextButton = Instantiate(this._next_btnPrefab, pageSelectionIcons);
            // also container with selection images must be defined and must have exatly the same amount of items as pages container
            if (pageSelectionIcons == null || _pageSelectionImages.Count != _pageCount)
            {
    ...
    public void SetPageSkipToLast()
    {
        // nothing to change
        if (_previousPageSelectionIndex == _pageCount-1)
        {
            return;
        }
        ...
        // select new
        _pageSelectionImages[_pageCount - 1].sprite = selectedPage;
        //_pageSelectionImages[aPageIndex].SetNativeSize();
    ...
    public void OnDrag(PointerEventData aEventData) 
    {
        ...
        else 
        {
            ...
            {
                difference = _startPosition.x - _container.anchoredPosition.x;
                int offSetX = (int)_scrollRectRect.rect.width / 2;
                if (difference > offSetX)
                {
                    aEventData.pointerDrag = null;
                    NextScreen();
                }
                if (difference < -offSetX)
                {
